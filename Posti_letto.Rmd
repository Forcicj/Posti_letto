---
title: "Decrescenza dei posti letto del SSN da 2010 al 2016"
...
**ordinari**: posti letto attivi al 1? gennaio dell'anno di riferimento dedicati
all'ttivit? di ricovero in regime ordinario e a pagamento.   
**day_hospital** posti letto attivi al 1? gennaio dell'anno di riferimento dedicati all'attivit? di ricovero in regime di day hospital (ricovero diurno di
tipo medico).   
**day_surgery** posti letto attivi al 1? gennaio dell'anno di riferimento dedicati all'attivit? di ricovero in regime di day surgery (ricovero diurno di
tipo chirurgico).   
...
>Come è stata applicata la riduzione forzata dei posti letto nelle regioni?
...
ggplot(regioni_new, aes(x =anno, y = posti_su_milione, group = regione, color = macro_area)) +
    geom_line(size = 1.3) +
    theme_minimal() + labs(title =  paste("      Ci sono differenze tra nord e centro-sud?"), x = paste(""), y = paste("Numero posti letto nella regione per un milione di residenti")) + theme(plot.title = element_text(face="italic", size=16, color= "#003333"), legend.title=element_blank(), axis.title.y = element_text(face="italic", size = 10, color= "#003333")) + geom_label(aes(label =regione), data = fascia)
```
...
*Ci sono delle differenze tra nord e centro-sud?*  
```{r , echo = FALSE}
residui %>%
...
*Variabilità spiegata dai modelli tramite R^2*  
```{r , echo = FALSE}
...
Non sempre i posti letto delle regioni diminuiscono ed è questo il caso delle tre regioni del nord (FVG, Emilia Romagna e Lombardia) e della regione Molise, del centro sud. Ma l'andamento negli anni tra le regioni del nord e la regione MOlise è praticamente opposto: quando i primi crescono, Molise decresce e viceversa.
Mentre la particolarità della regione Calabria è¨ che seppure decresce ogni anno, tra il 01.01.2011 e il 01.01.2013, quindi in due anni, la decrescita è stata molto forte rispetto alle altre regioni.
